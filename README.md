...
首页首屏直接展示“基础示例”分组，当前主要用于单独验证可复用组件和常见交互，不需要先从业务页兜进去。动感音频正在播放提示条固定展示在“基础示例”和“业务模块”之间，避免被业务模块展开状态影响，并与上下分组保持相同横向宽度。
...
- `Fluro`
- `Request`
- `Profile`（用户信息直接用 `AppField` 表单编辑，空值展示“请输入xx”占位，不再通过弹窗修改年龄）
- `Scrollable Tabs`
- `Network Img`
- `App Asset Icon`
- `App Sheet`
...
通用 Auth 登录页已接入 `/auth/login` route，仅保留手机号验证码登录，包含手机号/验证码输入限制、验证码倒计时、协议勾选、提交按钮启用态和关注页登录入口跳转。
